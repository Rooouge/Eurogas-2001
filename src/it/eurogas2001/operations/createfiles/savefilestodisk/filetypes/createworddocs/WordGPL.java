...
public class WordGPL extends EG2001File {

	public WordGPL(String pathForSave, InformationClass row, String date, String month) throws Throwable {
	...
	public void work(String pathForSave, InformationClass row, String date, String month) throws Throwable{
		try (
			...
			run.setFontSize(CreateWordUtils.FONT_SIZE_10);
			run.setText(s);
			s = "revisione decennale serbatoio G.P.L.";
			run = p.createRun();
			run.setBold(true);
			run.setFontSize(CreateWordUtils.FONT_SIZE_10);
			run.setText(s);
			...
			run = p.createRun();
			s = "Eurogas 2001 s.r.l. pensa di farLe cosa gradita nel ricordarLe che è in scadenza nel mese di ";
			run.setFontSize(CreateWordUtils.FONT_SIZE_10);
			run.setText(s);
			run = p.createRun();
			s = month;
			...
			run.setText(s);
			run = p.createRun();
			s = " il serbatoio G.P.L. installato sulla Sua vettura.";
			run.setText(s);
			run.addCarriageReturn();
			
			run = p.createRun();
			s = "Se lo riterrà opportuno potrà recarsi presso le nostre officine a disposizione per il cambio del serbatoio e per la eventuale revisione periodica dell'auto, oppure può contattarci direttamente presso le nostre sedi o al numero verde ";
			run.setFontSize(CreateWordUtils.FONT_SIZE_10);
			run.setText(s);
			s = "800 977 974";
			run = p.createRun();
			...
			file = new File(pathForSave + "[G.P.L.] " + row.name + " " + row.index + ".doc");
			
			int i = 1;
			while(file.exists()) {
				i++;
				file = new File(pathForSave + "[G.P.L.] " + row.name + " " + row.index + "(" + i + ").doc");
			}
