...
public class TxtGPL extends EG2001File {

	public TxtGPL(String pathForSave, InformationClass row) {
		File dir = new File(pathForSave);
		if(!dir.exists()) dir.mkdirs();
		
		row.name = Utils.removeInvalidCharacters(row.name);
		
		file = new File(pathForSave + "[G.P.L. - " + row.index + "]" + row.name + ".txt");
		
		int i = 1;
		while(file.exists()) {
			i++;
			file = new File(pathForSave + "[G.P.L. - " + row.index + "]" + row.name + "(" + i + ").txt");
		}
